...
export default function NavigationDrawer({
  ...
  return (
    <>
      ...
      <button
        className="fixed top-3 left-3 z-[60] lg:hidden p-2.5 rounded-xl bg-surface-container-lowest shadow-md2 hover:bg-surface-container-high transition-colors border border-outline-variant"
      ...
      {/* Sidebar drawer */}
      <aside
        className={`fixed top-0 left-0 bottom-0 w-[280px] bg-surface-container-lowest border-r border-outline-variant z-[80] flex flex-col overflow-y-auto transition-transform duration-300 ease-md ${
          open ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0`}
      >
        {/* Brand header */}
        <div className="p-5 pb-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-primary text-on-primary flex items-center justify-center font-bold text-lg shadow-md1 shrink-0">
            B3
          </div>
          ...
          <button
            className="lg:hidden p-1.5 rounded-lg hover:bg-surface-container-high text-surface-on-variant transition-colors shrink-0"
            onClick={() => setOpen(false)}
          ...
          {menu.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setOpen(false)}
                className={`flex items-center gap-3 px-4 h-12 rounded-xl font-medium text-sm transition-all duration-200 my-0.5 group ${
                  active
              ...
              >
                <Icon
                  ...
                  className={`transition-colors shrink-0 ${
                    active
                      ? "text-primary"
                      : "text-surface-on-variant group-hover:text-surface-on"
                  }`}
                />
        ...
        <div className="p-3 border-t border-outline-variant">
          <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-surface-container-high transition-colors group">
            ...
            <button
              ...
              title="Keluar"
              aria-label="Keluar"
              className="p-2 rounded-lg hover:bg-error-container text-surface-on-variant hover:text-error transition-colors shrink-0"
            >
              <LogOut size={18} />
            </button>
          </div>
